...
function SortHeader({ field, label, filters, onChange, align = "left" }) {
  ...
  return (
    <button
      ...
      className={`flex w-full items-center gap-1 text-xs font-medium uppercase tracking-wide text-slate-500 hover:text-slate-700 ${
...
// -----------------------------------------------------------------------
// Inline editable "Extra" cell.
// The addExtraPackagingData endpoint *increments* the stored value rather
// than overwriting it, so this control asks for a delta (+/-) to add, not
// a replacement number. It pulls straight from the store (same
// individual-selector convention as the rest of this file) so JobsTable
// doesn't need to thread any extra props through.
// -----------------------------------------------------------------------
function ExtraCell({ job }) {
  const [editing, setEditing] = useState(false);
  const [value, setValue] = useState("");

  const extraUpdatingId = usePackagingJobStore((s) => s.extraUpdatingId);
  const extraUpdateError = usePackagingJobStore((s) => s.extraUpdateError);
  const addExtraPackaging = usePackagingJobStore((s) => s.addExtraPackaging);

  const isSaving = extraUpdatingId === job.id;
  ...
  const submit = async (e) => {
    ...
    if (!value || Number.isNaN(delta) || delta === 0) return;

    const ok = await addExtraPackaging(job.id, delta);
    if (ok) {
      setEditing(false);
      setValue("");
    }
    ...
    return (
      ...
      >
        ...
        <button
          ...
          title="Add extra"
        >
          <Plus className="cursor-pointer h-3.5 w-3.5" />
        </button>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-1" onClick={(e) => e.stopPropagation()}>
      <div className="flex items-center gap-1">
        <input
          ...
          placeholder="±qty"
          disabled={isSaving}
          className="w-20 rounded-md border border-slate-300 px-2 py-1 text-sm tabular-nums focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 disabled:opacity-50"
        />
        <button
          type="button"
          onClick={submit}
        ...
        >
          {isSaving ? (
            ...
            <Check className="cursor-pointer h-3.5 w-3.5" />
          )}
        </button>
        <button
          type="button"
          ...
          title="Cancel"
        >
          <X className="cursor-pointer h-3.5 w-3.5" />
        </button>
      </div>
      {extraUpdateError && (
        <span className="text-[11px] text-red-500">{extraUpdateError}</span>
      )}
    </div>
  );
}
