...
const CorporateOrderEdit = () => {
  ...
  const updateCorporateOrder = corporateDataStore(
    (state) => state.updateCorporateOrder,
  );
  ...
  const loadOrder = async () => {
    try {
      ...
      setCorporateData({
        productName: data.orderDetails?.productName || "",
        description: data.orderDetails?.description || "",
        qty: data.orderDetails?.qty || "",
        unit: data.orderDetails?.unit || "L",
        pricePerUnit: data.orderDetails?.pricePerUnit || "",
      ...
      console.error(err);
      toast.error("Unable to load corporate order.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!corporateData.pricePerUnit || Number(corporateData.pricePerUnit) <= 0) {
    ...
    try {
      ...
      const payload = {
        orderDetails: {
          ...
          orderTotal: Number(corporateData.qty) * Number(corporateData.pricePerUnit),
      ...
      await updateCorporateOrder(id, payload);
      ...
      console.error(err);
      toast.error("Failed to update corporate order");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      {/* Product Details Form Starts Here */}
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        <div className="flex flex-col sm:flex-row justify-end gap-3">
          <button
            type="button"
            onClick={() => navigate(-1)}
            className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 transition"
          >
            <X size={16} />
            Cancel
          </button>
